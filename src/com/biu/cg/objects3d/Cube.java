...
public class Cube extends Polyhedron {
	 ...
	 * @return
	 */
	public static Cube createCube(Vector position, Vector axis, float angle, float size, String textureSrc) {
	 ...
	 * @param textureSrc - image for the texture
	 */
	protected Cube(Vector position, Vector axis, float angle, float size, String textureSrc) {
		...
		vertices[0].v = new Vector(-1f*size,-1f*size,-1f*size);
		vertices[1].v = new Vector(1f*size, -1f*size,-1f*size);
		vertices[2].v = new Vector(1f*size, -1f*size, 1f*size);
		vertices[3].v = new Vector(-1f*size,-1f*size, 1f*size);
		vertices[4].v = new Vector(-1f*size, 1f*size,-1f*size);
		vertices[5].v = new Vector(1f*size,  1f*size,-1f*size);
		vertices[6].v = new Vector(1f*size,  1f*size, 1f*size);
		vertices[7].v = new Vector(-1f*size, 1f*size, 1f*size);
	...
	protected void synchronizedDraw(GLAutoDrawable gLDrawable) {
		...
		float x = orientation.getPosition().getX();
		float y = orientation.getPosition().getY();
		float z = orientation.getPosition().getZ();
		...
		gl.glTexCoord2f(1f, 1f); 
		gl.glColor3f(c3, c1, c2);
		gl.glVertex3f(vertices[5].u.getX()+x, vertices[5].u.getY()+y, vertices[5].u.getZ()+z);
		
		gl.glTexCoord2f(0f, 1f); 
		gl.glColor3f(c3, c1, c2);
		gl.glVertex3f(vertices[4].u.getX()+x, vertices[4].u.getY()+y, vertices[4].u.getZ()+z);
		...
		gl.glTexCoord2f(1f, 1f); 
		gl.glColor3f(c2, c3, c1);
		gl.glVertex3f(vertices[6].u.getX()+x, vertices[6].u.getY()+y, vertices[6].u.getZ()+z);
		...
		gl.glTexCoord2f(1f, 1f); 
		gl.glColor3f(c2, c1, c3);
		gl.glVertex3f(vertices[7].u.getX()+x, vertices[7].u.getY()+y, vertices[7].u.getZ()+z);
